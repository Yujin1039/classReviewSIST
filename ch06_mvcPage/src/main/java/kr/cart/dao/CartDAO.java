...
public class CartDAO {
	...
	public List<CartVO> getListCart(int mem_num) throws Exception{
		...
		try {
			//커넥션풀로부터 커넥션 할당
			conn = DBUtil.getConnection();
			//SQL문 작성 - zcart, zitem 모두 reg_date 컬럼이 있으므로 구분하기 위해 알리아스 부여
			...
			while(rs.next()) {
				...
				//동일상품(item_num이 같은 상품)의 총 구매 금액 구하기
				//-> VO에만 저장하고 DB에는 저장하지 않음
	...
	//장바구니 수정(상품번호와 회원번호별 수정) -> 컬럼 2개 이용
