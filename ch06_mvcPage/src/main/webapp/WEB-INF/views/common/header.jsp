...
<div id="main_nav">
	<ul>
		...
		<c:if test="${!empty user_num && user_auth == 9}"><!-- 관리자가 로그인한 경우 -->
			<li>
		...
		<c:if test="${!empty user_num}"><!-- 로그인이 되어있는 경우 -->
		<li>
		...
		<c:if test="${!empty user_num && !empty user_photo}"><!-- 로그인O, 프로필사진O -->
		<li class="menu-profile">
			<img src="${pageContext.request.contextPath}/upload/${user_photo}" 
					width="25" height="25" class="my-photo">
		</li>
		</c:if>
		<c:if test="${!empty user_num && empty user_photo}"><!-- 로그인O, 프로필사진X -->
		<li class="menu-profile">
			<img src="${pageContext.request.contextPath}/images/face.png" 
					width="25" height="25" class="my-photo">
		</li>
		</c:if>
		<c:if test="${!empty user_num}"><!-- 로그인O일때, 로그아웃 메뉴 -->
		<li class="menu-logout">
			[<span>${user_id}</span>]
			<a href="${pageContext.request.contextPath}/member/logout.do">로그아웃</a>
		</li>
		</c:if>
		<c:if test="${empty user_num}"><!-- 로그인이 되어있지 않은 경우 -->
		<li>
