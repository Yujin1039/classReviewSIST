...
public class ItemDAO {
	...
	public void updateItem(ItemVO item)throws Exception{
		...
		try {
			...
			sql = "UPDATE zitem SET name=?,price=?,quantity=?,detail=?,modify_date=SYSDATE"
					+ ",status=?" + sub_sql + " WHERE item_num=?";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
	...
	public List<ItemVO> getListItem(
			int start,int end,String keyfield,String keyword,int status)throws Exception{
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
