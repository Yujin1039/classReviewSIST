...
public class OrderDAO {
	...
	public void insertOrder(OrderVO order, List<OrderDetailVO> orderDetailList) throws Exception{
		...
		try {
			...
			//주문 정보 처리
			sql = "INSERT INTO zorder (order_num,order_total,payment,receive_name,"
					+ "receive_post,receive_address1,receive_address2,receive_phone,"
					+ "notice,mem_num) VALUES (?,?,?,?,?,?,?,?,?,?)";
			...
			//주문 상세정보 처리
			sql = "INSERT INTO zorder_detail (detail_num,item_num,item_name,item_price,"
			...
			for(int i=0;i<orderDetailList.size();i++) {
				...
				//배치: sql문의 실행결과를 DB에 바로 반영하는 것이 아니라 메모리에 약 1000개까지 정보를 저장해두었다가 DB에 반영
	...
	//주문 삭제(삭제시 재고를 원상복귀시키지 않음, 주문 취소일 때 재고 수량 원상복귀)
	//관리자/사용자 - 주문 상세
	...
	public void updateOrder(OrderVO order)throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "UPDATE zorder SET receive_name=?,receive_post=?,receive_address1=?,"
